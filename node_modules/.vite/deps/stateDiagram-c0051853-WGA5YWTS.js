...
// node_modules/mermaid/dist/stateDiagram-c0051853.js
var L3 = {};
var $ = (e, i) => {
  L3[e] = i;
};
var v = (e) => L3[e];
var P = () => Object.keys(L3);
var I = () => P().length;
var _ = {
  get: v,
  set: $,
...
var K = (e, i) => {
  const o = function(l, m, w) {
    ...
    w || k.attr("dy", Ft().state.textHeight);
...
var at = function(e, i, o) {
  ...
  const s = i.points, g = S().x(function(r) {
    ...
    return r.y;
  }).curve(Nf), p = e.append("path").attr("d", g(s)).attr("id", "edge" + A).attr("class", "transition");
  let a = "";
  if (Ft().state.arrowMarkerAbsolute && (a = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, a = a.replace(/\(/g, "\\("), a = a.replace(/\)/g, "\\)")), p.attr(
    ...
    const r = e.append("g").attr("class", "stateLabel"), { x: y, y: x } = Xe.calcLabelPosition(i.points), c = In.getRows(o.title);
    let l = 0;
    const m = [];
    let w = 0, k = 0;
    for (let u = 0; u <= c.length; u++) {
      const h = r.append("text").attr("text-anchor", "middle").text(c[u]).attr("x", y).attr("y", x + l), f = h.node().getBBox();
      w = Math.max(w, f.width), k = Math.min(k, f.x), L2.info(f.x, y, x + l), l === 0 && (l = h.node().getBBox().height, L2.info("Title height", l, x)), m.push(h);
    }
    let E = l * c.length;
    if (c.length > 1) {
    ...
    r.insert("rect", ":first-child").attr("class", "box").attr("x", y - w / 2 - Ft().state.padding / 2).attr("y", x - E / 2 - Ft().state.padding / 2 - 3.5).attr("width", w + Ft().state.padding).attr("height", E + Ft().state.padding), L2.info(n);
  }
  A++;
};
var B;
...
var W = (e, i, o, d, s, g, p) => {
  ...
  L2.debug("Count=", a.nodeCount(), a);
  let m = 0;
  c.forEach(function(n) {
    m++, L2.debug("Setting edge", n), a.setEdge(
      n.id1,
      n.id2,
      {
        relation: n,
  ...
  }), js(a), L2.debug("Graph after layout", a.nodes());
  const w = i.node();
  a.nodes().forEach(function(n) {
    n !== void 0 && a.node(n) !== void 0 ? (L2.warn("Node " + n + ": " + JSON.stringify(a.node(n))), s.select("#" + w.id + " #" + n).attr(
      "transform",
    ...
    })) : L2.debug("No Node " + n + ": " + JSON.stringify(a.node(n)));
  });
  let k = w.getBBox();
  a.edges().forEach(function(n) {
    n !== void 0 && a.edge(n) !== void 0 && (L2.debug("Edge " + n.v + " -> " + n.w + ": " + JSON.stringify(a.edge(n))), at(i, a.edge(n), a.edge(n).relation));
  }), k = w.getBBox();
  const E = {
    id: o || "root",
    label: o || "root",
  ...
  return E.width = k.width + 2 * B.padding, E.height = k.height + 2 * B.padding, L2.debug("Doc rendered", E, a), E;
};
var ot = {
  setConf: it,
  draw: st
...
//# sourceMappingURL=stateDiagram-c0051853-WGA5YWTS.js.map
